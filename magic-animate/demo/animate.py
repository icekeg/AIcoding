...
class MagicAnimate():
    ...
    def __call__(self, source_image, motion_sequence, random_seed, step, guidance_scale, size=512):
            ...
            result_per_video = []
            result_per_video.append(sample[:, :, :original_length])
            result_per_video = torch.cat(result_per_video)

            time_str = datetime.datetime.now().strftime("%Y-%m-%dT%H-%M-%S")
            savedir = f"demo/outputs"
            animation_path = f"{savedir}/{time_str}.mp4"
            animation_result_path = f"{savedir}/{time_str}_result.mp4"

            os.makedirs(savedir, exist_ok=True)
            save_videos_grid(samples_per_video, animation_path)
            save_videos_grid(result_per_video,animation_result_path)
            
            return animation_path, animation_result_path
