...
<html>
  ...
  <body>
    <div style="color: #cccccc; background-color: #1f1f1f; font-family: Consolas, Courier New, monospace; font-weight: normal; font-size: 14px; line-height: 19px; white-space: pre">
      <div>
        ImageDiffusionWidget作为dockwidget，调用四个不同的Qwidget[

        <div style="color: #cccccc; background-color: #1f1f1f; font-family: Consolas, Courier New, monospace; font-weight: normal; font-size: 14px; line-height: 19px; white-space: pre">
          <div>
            <p>
              GenerationWidget,UpscaleWidget,LiveWidget,WelcomeWidget]
            </p>
          </div>
        </div>
      </div>
...
<html>
  ...
  <body>
    <p>
      唯一根对象，用于维护所有实例，跟踪文档状态并为其创建model
    </p>
    <p>
      init()函数用于连接服务器，清空model状态，绑定收到信息与处理信息的方法，model_for_active_document()方法用于激活图像的模式创建（在difuusion中注册实例，实例有变动会通知diffusion的update函数）
    </p>
    <p>
